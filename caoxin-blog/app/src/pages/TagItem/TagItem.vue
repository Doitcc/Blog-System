<template>
	<div class="main">
      ...
      <div class="page-container null" v-else>
        <div class="empty">
...
export default {
  ...
  watch:{
    $route: {
      handler(to,from) {
        console.log('tagitem跳转路由',to,from);
        if(to.name=='tagitem') {
        ...
        // if(to.name==from.name&&to.params.tagId==from.params.tagId) {
        //   to.params.tagName=from.params.tagName
        // }
      },
      immediate: true
    }
  ...
  methods: {
    ...
    async getArticleList() {
      ...
      const res = await this.$API.manage.getTagArticleListByTagId(params)
      console.log(`查询该${this.tagName}标签展示文章`,res,tagId);
      if(res.code==200) {
